...
unsigned int destination[SIZE];
void dijkstra(unsigned int start,unsigned int nodes,unsigned int*path,int*table[SIZE]);
...
class SDN_stat_payload : public payload {
    ...
    public:
        ...
        //the vector of neighbors of each nodes
        SET(setNeiID,vector<cost>,neighbors,_neighbors);
        GET(getNeiID,vector<cost>,neighbors);
...
class node {
    ...
    public:
        int*checker[SIZE];
        int stat_checker[SIZE];
        int*table[SIZE];
        unsigned int path[SIZE];
        map<unsigned int,unsigned int>result;
        map<unsigned int,unsigned int>whole_result[SIZE];
        //void dijkstra(unsigned int start,unsigned int nodes,unsigned int*path,int*table[SIZE]);
    ...
    public:
        ...
        class pkt_gen_data{
            public:
                unsigned int src_id; // the controller
                unsigned int dst_id; // the node that should update its rule
                vector<cost>neighbors; //the neighbor of the node
                string msg;
                // packet *_pkt;
        };
...
unsigned int SDN_stat_pkt_gen_event::event_priority() const {
    string string_for_hash;
    string_for_hash = to_string(getTriggerTime())+ to_string(src) + to_string(dst) ;//+ to_string(neighbors[0].id); //to_string (pkt->getPacketID());
    return get_hash_value(string_for_hash);
}
...
void stat_packet_event (unsigned int con_id, unsigned int id,vector<cost>neighbors, unsigned int t = event::getCurTime(), string msg = "default"){
// void ctrl_packet_event (unsigned int dst, unsigned int t = 0, string msg = "default"){
    if ( id == BROCAST_ID || node::id_to_node(id) == nullptr ) {
        cerr << "id is incorrect" << endl; return;
    }
...
void SDN_switch::recv_handler (packet *p){
    ...
    if (p->type() == "SDN_data_packet") { // the switch receives a packet from the other switch
        ...
        if(getNodeID()!=p->getHeader()->getDstID()){
            ...
            if(result.find(p->getHeader()->getDstID())==result.end()){
                return;
            }
            SDN_data_packet * p2 = nullptr;
            p2 = dynamic_cast<SDN_data_packet*> (p);
            p2->getHeader()->setPreID ( getNodeID() );
            //cout<<"*"<<getNodeID()<<" "<<p->getHeader()->getDstID()<<" "<<result[p->getHeader()->getDstID()]<<endl;
            p2->getHeader()->setNexID ( result[p->getHeader()->getDstID()] );
        ...
        string msg = l3->getMsg();// get the msg
        result[mat]=act;
        //cout<<getNodeID()<<" "<<mat<<" "<<act<<endl;
        //send ack packet back to controller
        SDN_ctrl_packet * p4 = nullptr;
        p4 = dynamic_cast<SDN_ctrl_packet*> (p);
        p4->getHeader()->setNexID ( p->getHeader()->getPreID() );
        p4->getHeader()->setDstID ( p->getHeader()->getPreID() );
        p4->getHeader()->setPreID ( getNodeID() );
        send_handler (p4);
    }
    else if (p->type() == "SDN_stat_packet") { // the switch receives a packet from the controller
        SDN_stat_packet *p5 = nullptr;
        p5 = dynamic_cast<SDN_stat_packet*> (p);
        //string msg = l3->getMsg(); // get the msg
        //cout << getNodeID() << " received packet with msg context \"" << msg << "\""<< endl;

        //send SDN_stat_packet to controller
        p5->getHeader()->setNexID( p->getHeader()->getDstID() );
        send_handler (p5);
    }
...
void SDN_controller::recv_handler (packet *p){
    ...
    if(p->type() == "SDN_data_packet"){
        ...
        p2 = dynamic_cast<SDN_data_packet*> (p);
        //p2->getHeader()->setPreID ( getNodeID() );
        p2->getHeader()->setPreID ( p->getHeader()->getSrcID() );
        p2->getHeader()->setNexID ( p->getHeader()->getSrcID() );
        p2->getHeader()->setDstID ( p->getHeader()->getDstID() );
        send_handler (p2);
    }
    else if (p->type() == "SDN_ctrl_packet") { // the switch receives a packet from the controller
        ...
        // 0:none 1:sent 2:recieve ack

        // if recieve an ack
        if(p->getHeader()->getDstID() == getNodeID()){
            //cout<<msg<<endl;
            checker[mat][(p->getHeader()->getPreID())]=2;
        }

        // send all the ctrl_packet?
        flag=0;
        for(unsigned int i=0;i<getNodeNum()-1;i++){
            if(checker[mat][i]!=2){
                flag=1;
                break;
            }
        ...
        if(flag==0){
            //cout<<"InsTime is over"<<endl;
            for(unsigned int i=0;i<getNodeNum()-1;i++){
                for(unsigned int j=0;j<getNodeNum()-1;j++){
                    checker[i][j]=0;
                }
            }
        }

        if(checker[mat][dst]==0 && dst!=getNodeID()){
            if(event::getCurTime()==InsTime+10){
                if(mat==act){
                    checker[mat][mat]==2;
                }
                ctrl_packet_event(getNodeID(),dst,mat,act);
                checker[mat][dst]=1;
            }else{
                if(mat==act){
                    checker[mat][mat]==2;
                    ...
                    //t.msg=msg;
                    round.push_back(t);
                    //cout<<"waiting "<<l5->getMatID()<<" "<<l5->getActID()<<endl;
                    //cout<<"waiting "<<t.mat<<" "<<t.act<<endl;
                }
            }
        }else if(checker[mat][dst]==1 && dst!=getNodeID()){
            SDN_ctrl_packet * p4 = nullptr;
            p4 = dynamic_cast<SDN_ctrl_packet*> (p);
            p4->getHeader()->setPreID ( getNodeID() );
            p4->getHeader()->setNexID ( p->getHeader()->getDstID() );
            p4->getHeader()->setDstID ( p->getHeader()->getDstID() );
            send_handler (p4);
        }

        //cout<<round.size()<<endl;
        for(int i=0;i<round.size();i++){
            if(checker[round[i].mat][round[i].act]==2 && checker[round[i].mat][round[i].dst]==0){
                ctrl_packet_event(getNodeID(),round[i].dst,round[i].mat,round[i].act);
                checker[round[i].mat][round[i].dst]=1;
            }
        }
    }
        ...
        SDN_stat_packet *p5 = nullptr;
        p5 = dynamic_cast<SDN_stat_packet*> (p);
        SDN_stat_payload *l5 = nullptr;
        l5 = dynamic_cast<SDN_stat_payload*> (p5->getPayload());
        vector<cost>neighbors=l5->getNeiID();
        
        stat_checker[p5->getHeader()->getPreID()]=1;
        for(int i=0;i<neighbors.size();i++){
            table[p5->getHeader()->getPreID()][neighbors[i].id]=neighbors[i].W;
            table[neighbors[i].id][p5->getHeader()->getPreID()]=neighbors[i].W;
        }
        flag=0;
        for(unsigned int i=0;i<getNodeNum()-1;i++){
            if(stat_checker[i]==0){
        ...
        if(flag==0){
            //cout<<"all stat receive"<<endl;
            //void dijkstra(unsigned int start,unsigned int nodes,unsigned int*path,int*table[SIZE])
            for(unsigned int i=0;i<dsts;i++){
                dijkstra(destination[i],nodes,path,table);
                for(unsigned int j=0;j<nodes;j++){
                    if(j==destination[i]){
                        continue;
                    }
                    whole_result[j][destination[i]]=path[j];
                }
            }
            for(unsigned int i=0;i<dsts;i++){
                for(unsigned int j=0;j<nodes;j++){
                    ...
                    //ctrl_packet_event(getNodeID(),dst,mat,act)
                    ctrl_packet_event(getNodeID(),j,destination[i],whole_result[j][destination[i]]);
                }
            }
            for(unsigned int i=0;i<getNodeNum()-1;i++){
                stat_checker[i]=0;
...
void dijkstra(unsigned int start,unsigned int nodes,unsigned int*path,int*table[SIZE]){
    unsigned int*dis=new unsigned int[SIZE];
    unsigned int tmp,t;
    ...
    for(unsigned int i=0;i<nodes;i++){
        ...
        visited[t]=1;
        const map<unsigned int,bool> &nblist = node::id_to_node(t)->getPhyNeighbors();
        for (map<unsigned int,bool>::const_iterator it = nblist.begin(); it != nblist.end(); it ++) {
            //it->first=neighbors
            ...
            w=table[t][it->first];
            if((dis[t]+w)<dis[it->first]){
                dis[it->first]=dis[t]+w;
                path[it->first]=t;
            //if same shorest path
            }else if((dis[t]+w)==dis[it->first]){
                if(t<path[it->first]){
                    path[it->first]=t;
                }
            }
...
int main()
{
    ...
    unsigned int c;
    //unsigned int*path=new unsigned int[SIZE];
    int oldW,newW;
    cost tmp;
    vector<cost>neighbors_old[SIZE];
    vector<cost>neighbors_new[SIZE];
    ...
    for(unsigned int i=0;i<links;i++){
        ...
        tmp.id=a;
        tmp.W=oldW;
        neighbors_old[b].push_back(tmp);
        tmp.id=a;
        tmp.W=newW;
        neighbors_new[b].push_back(tmp);
        tmp.id=b;
        tmp.W=oldW;
        neighbors_old[a].push_back(tmp);
        tmp.id=b;
        tmp.W=newW;
        neighbors_new[a].push_back(tmp);
    }

    //set conroller's neighbors
    for(unsigned int i=0;i<nodes;i++){
    ...
    //store data_packet_event
    while((cin>>t>>src>>dst)){
        data_packet_event(src, dst, t);
    }

    for(int i=0;i<nodes;i++){
        node::id_to_node(con_id)->checker[i]=new int[nodes];
        node::id_to_node(con_id)->table[i]=new int[nodes];
    }

    // initialize checker
    for(unsigned int i=0;i<nodes;i++){
        for(unsigned int j=0;j<nodes;j++){
            node::id_to_node(con_id)->checker[i][j]=0;
        }
        node::id_to_node(con_id)->stat_checker[i]=0;
    }
    for(unsigned int i=0;i<dsts;i++){
        for (unsigned int id = 0; id < node::getNodeNum()-1; id ++){
            if(id==destination[i]){
                node::id_to_node(con_id)->checker[id][id]=2;
                continue;
            }
        }
    }
    ...
    //stat_packet_event
    for(unsigned int i=0;i<nodes;i++){
        stat_packet_event(con_id,i,neighbors_old[i],InsTime);
        stat_packet_event(con_id,i,neighbors_new[i],UpdTime);
    }
