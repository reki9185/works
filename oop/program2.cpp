...
//store all the data_packet_event(sort)
class events{
public:
    unsigned int t;
    unsigned int src;
...
//store all the crtl_packet_event(sort)
class pac{
public:
    unsigned int id;
    string msg;
...
void SDN_switch::recv_handler (packet *p){
    ...
    /*if(event::getCurTime()>SimDuration){
        return;
    }*/
    ...
    if (p->type() == "SDN_data_packet") { // the switch receives a packet from the other switch
        ...
        for(unsigned int i=0;i<len;i++){
            if(msg[i]!=' '){
                ...
                dst=stoul(tmp,nullptr,10);
                tmp.clear();
            }
        }
        next=stoul(tmp,nullptr,10);
        result[dst]=next;
        //cout<<"+++++"<<getNodeID()<<":"<<" "<<dst<<" "<<node::result[dst]<<endl;
    }
...
bool cmp_data(events a,events b){
    if(a.t==b.t){
        if(a.src==b.src){
            return a.dst<b.dst;
        }
...
int main()
{
    ...
    sort(input,input+times,cmp_data);
    ...
    //generate data_packet_event before InsTime
    for(unsigned int i=0;i<times;i++){
        if(input[i].t<InsTime){
            data_packet_event(input[i].src,input[i].dst,input[i].t);
        }
    }
    ...
    sort(tmp,tmp+cnt,cmp_ctrl);

    for(unsigned int i=0;i<cnt;i++){
        ctrl_packet_event(tmp[i].id,InsTime,tmp[i].msg);
    }

    //generate data_packet_event<UpdTime
    for(unsigned int i=0;i<times;i++){
        if(input[i].t<InsTime){
            continue;
        }
        if(input[i].t<UpdTime){
            data_packet_event(input[i].src,input[i].dst,input[i].t);
        }
    }
    ...
    for(unsigned int i=0;i<dsts;i++){
        for (unsigned int id = 0; id < node::getNodeNum(); id ++){
            ...
            if(new_result[id][destination[i]]!=old_result[id][destination[i]]){
    ...
    for(unsigned int i=0;i<cnt;i++){
        ctrl_packet_event(tmp[i].id,UpdTime,tmp[i].msg);
    }
    ...
    // start simulation!!
    // !!!!!!not coming back to main until sending to the destination!!!!!!!!
