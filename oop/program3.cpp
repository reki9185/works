...
void SDN_controller::recv_handler (packet *p){
    ...
    if(p->type() == "SDN_data_packet"){
        ...
        //first recieve the ctrl_packet
        if(checker[mat][dst]==0 && dst!=getNodeID()){
            if(event::getCurTime()==InsTime || msg!="#"){
                if(mat==act){
                    checker[mat][mat]==2;
                }
...
void dijkstra(unsigned int start,unsigned int nodes,int state,unsigned int*path,weight*table[SIZE]){
    ...
    for(unsigned int i=0;i<nodes;i++){
        ...
        for (map<unsigned int,bool>::const_iterator it = nblist.begin(); it != nblist.end(); it ++) {
            ...
            if(state==0){
                w=table[t][it->first].oldW;
            }else{
                w=table[t][it->first].newW;
            }
...
bool cycle(map<unsigned int,unsigned int>old_result[SIZE],unsigned int id,unsigned int act,unsigned int dst,bool visited[SIZE]){
    unsigned int tmp=act;
    int flag=0;
    while(visited[tmp]!=1){
        if(tmp==dst){
            flag=1;
        ...
        //node::id_to_node(id)->before.push_back(tmp);
        visited[tmp]=1;
        tmp=old_result[tmp][dst];
    }
    if(flag==0){
        return 1;
    }else{
        //node::id_to_node(id)->before.clear();
        return 0;
    }
}
...
int main()
{
    ...
    int oldW,newW;
    int state=0;
    map<unsigned int,unsigned int>old_result[SIZE];
    map<unsigned int,unsigned int>new_result[SIZE];
    map<unsigned int,unsigned int>tmp[SIZE];
    weight*table[SIZE];
    ...
    for(unsigned int i=0;i<dsts;i++){
        dijkstra(destination[i],nodes,0,path,table);
        for(unsigned int j=0;j<nodes;j++){
            if(j==destination[i]){
                continue;
            }
            old_result[j][destination[i]]=path[j];
            tmp[j][destination[i]]=path[j];
        }
    }

    for(unsigned int i=0;i<dsts;i++){
        dijkstra(destination[i],nodes,1,path,table);
        for(unsigned int j=0;j<nodes;j++){
            if(j==destination[i]){
                continue;
            }
            new_result[j][destination[i]]=path[j];
        }
    }
    ...
    for(unsigned int i=0;i<dsts;i++){
        ...
        // for (int id = node::getNodeNum()-1; id >= 0; id --){ // this line is used to check whether the sequence affects the results
            ...
            //string msg = to_string(destination[i]) + " " + to_string(old_result[id][destination[i]]);
            ctrl_packet_event(con_id,id,destination[i],old_result[id][destination[i]],InsTime);
        ...
        // !!!! to generate the new ctrl msg in the next round
         ...
         /*for(unsigned int i=0;i<dsts;i++){
            for (unsigned int id = 0; id < node::getNodeNum()-1; id ++){
                act=old_result[id][destination[i]];
                if(id==destination[i]){
                    node::id_to_node(con_id)->checker[id][id]=2;
                    continue;
                }
                if(old_result[id][destination[i]]==new_result[id][destination[i]]){
                    //string msg = to_string(destination[i]) + " " + to_string(new_result[id][destination[i]]) + "!";
                    ctrl_packet_event(con_id,id,destination[i],new_result[id][destination[i]],UpdTime,"!");
                    continue;
                }
                //old_result[id][destination[i]]=new_result[id][destination[i]];
                ctrl_packet_event(con_id,id,destination[i],new_result[id][destination[i]],UpdTime,"#");
            }
        }*/
    ...
    for(unsigned int i=0;i<dsts;i++){
        for(unsigned int id=0;id<node::getNodeNum()-1;id++){
            ...
            tmp[id][destination[i]]=new_result[id][destination[i]];
            act=new_result[id][destination[i]];
            for(int j=0;j<nodes;j++){
                visited[j]=0;
            }
            visited[id]=1;
            c=cycle(old_result,id,act,destination[i],visited);
            if(c==1){
                //cout<<id<<" "<<destination[i]<<" "<<new_result[id][destination[i]]<<endl;
                //string msg = to_string(destination[i]) + " " + to_string(new_result[id][destination[i]]) + "#";
                //cout<<id<<" "<<msg<<endl;
                continue;
            }
            //old_result[id][destination[i]]=act;
            /*else if(id==old_result[(new_result[id][destination[i]])][destination[i]]){
                string msg = to_string(destination[i]) + " " + to_string(new_result[id][destination[i]]) + "#";
                //cout<<id<<" "<<msg<<endl;
                ctrl_packet_event(con_id,id,destination[i],new_result[id][destination[i]],UpdTime,msg);
            }*/
            //string msg = to_string(destination[i]) + " " + to_string(new_result[id][destination[i]]);
            for(int j=0;j<nodes;j++){
                visited[j]=0;
            ...
            visited[id]=1;
            c=cycle(tmp,id,act,destination[i],visited);
            if(c==1){
                cnt++;
            }
            //tmp[id][destination[i]]=new_result[id][destination[i]];
    ...
    for(unsigned int i=0;i<dsts;i++){
        for (unsigned int id = 0; id < node::getNodeNum()-1; id ++){
            /*if(id==destination[i]||old_result[id][destination[i]]==new_result[id][destination[i]]){
                continue;
            }*/
            act=old_result[id][destination[i]];
            if(id==destination[i]){
